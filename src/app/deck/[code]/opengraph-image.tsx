...
export default async function Image({ params }: { params: { code: string } }) {
  ...
  const cardObjects = cards.map(id => getCardById(id)).filter((c): c is NonNullable<typeof c> => c != null);
  ...
  return new ImageResponse(
    (
      ...
      >
        <div style={{ fontSize: 60, marginBottom: 40 }}>
          Ashenhall Deck: {factionName}
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', width: '90%', justifyContent: 'center' }}>
          {cardObjects.slice(0, 10).map((card, index) => (
            <div key={index} style={{ margin: 10, padding: 10, border: '1px solid #4B5563', borderRadius: 8, background: '#1F2937', width: 180 }}>
