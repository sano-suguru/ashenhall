...
describe('executeHandDiscardEffect - filterTargets統合テスト', () => {
  ...
  beforeEach(() => {
    ...
    const testDeck = [
      ...necromancerCards
        .slice(0, 5)
        .map((template, i) => createCardFromTemplate(template, `necro-test-${i}`)),
      ...mageCards
        .slice(0, 5)
        .map((template, i) => createCardFromTemplate(template, `mage-test-${i}`)),
      ...knightCards
        .slice(0, 5)
        .map((template, i) => createCardFromTemplate(template, `knight-test-${i}`)),
    ];
  ...
  describe('基本的な手札破壊機能', () => {
    it('フィルターなしで正しくカードを破壊する', () => {
      ...
      // アクションログに記録されることを確認
      const discardAction = gameState.actionLog.find(
        (action) => action.type === 'effect_trigger' && action.data.effectType === 'hand_discard'
      );
  ...
  describe('FilterRuleによるフィルタリング', () => {
    it('勢力フィルターで正しくカードを破壊する', () => {
      ...
      // メイジカードが破壊されたことを確認
      const discardedCard =
        gameState.players.player1.graveyard[gameState.players.player1.graveyard.length - 1];
    ...
    it('条件に合うカードがない場合は何もしない', () => {
      gameState.players.player1.hand = [
        createCardFromTemplate(necromancerCards[0], 'no-match-test'),
      ];
