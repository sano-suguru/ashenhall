...
import { UniversalFilterEngine } from "@/lib/game-engine/core/target-filter";
...
describe("UniversalFilterEngine - Card[]とFieldCard[]両対応テスト", () => {
  ...
  describe("Card[]フィルタリング (デッキ・手札用)", () => {
    ...
    it("勢力フィルターがCard[]で正しく動作する", () => {
      ...
      const result2 = UniversalFilterEngine.applyRules(cards, rules2);
  ...
  describe("FieldCard[]フィルタリング (場用)", () => {
    it("体力フィルターがFieldCard[]で正しく動作する", () => {
      ...
      const result = UniversalFilterEngine.applyRules(fieldCards, rules);
      expect(result).toHaveLength(1);

      const rules2: FilterRule[] = [{ type: 'health', operator: 'range', minValue: 5, maxValue: 7 }];
      const result2 = UniversalFilterEngine.applyRules(fieldCards, rules2);
      expect(result2).toHaveLength(0);
    });
    ...
    it("烙印フィルターがFieldCard[]で正しく動作する", () => {
      ...
      let result = UniversalFilterEngine.applyRules(fieldCards, rules);
      expect(result).toHaveLength(1);

      // 烙印ありの状態
      mockFieldCard.statusEffects = [{ type: 'branded' }];
      const rules2: FilterRule[] = [{ type: 'brand', operator: 'has' }];
      result = UniversalFilterEngine.applyRules(fieldCards, rules2);
      expect(result).toHaveLength(1);
    });
