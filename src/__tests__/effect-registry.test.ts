...
describe('resolveDynamicEffectParameters', () => {
  ...
  beforeEach(() => {
    gameState = createInitialGameState('test-game', [], [], 'knight', 'mage', 'test-seed');
  });
  ...
  it('necro_grave_giantの攻撃力バフが墓地のクリーチャー数と等しくなること', () => {
    ...
    const { value } = resolveDynamicEffectParameters(
      gameState,
      effect,
      sourceCard,
      sourcePlayerId,
  ...
  it('kni_white_wing_marshalが自身をバフの対象から除外すること - selectionRulesで処理', () => {
    ...
    const { targets } = resolveDynamicEffectParameters(
      gameState,
      effect,
      sourceCard,
      sourcePlayerId,
