...
describe('秘術の連雷 - 連鎖ダメージシステム', () => {
  ...
  test('アクションログに2回のダメージが記録される', () => {
    ...
    // ダメージアクションが2回記録されている
    const damageActions = state.actionLog.filter((action) => action.type === 'effect_trigger');

    const effectTriggerActions = damageActions.filter(
      (a) => a.data && 'effectType' in a.data && a.data.effectType === 'damage'
    );
  ...
  test('決定論性：同じシードで同じ結果', () => {
    ...
    const createTestWithSeed = () => {
      const testDeck = mageCards
        .slice(0, 6)
        .map((card, i) => ({ ...card, instanceId: `det-deck-${i}` }));
      const state = createInitialGameState('test-det', testDeck, testDeck, 'mage', 'mage', seed);
      ...
      const arcaneLightningTemplate = mageCards.find(
        (c) => c.templateId === 'mag_arcane_lightning'
      )!;
