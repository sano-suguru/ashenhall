...
function pushCreature(
  ...
  card: Partial<FieldCard> & { templateId: string; name: string }
): FieldCard {
...
describe('Death Sweep Edge Cases', () => {
  ...
  test('passive recalculation induced death is swept', () => {
    ...
    const card = pushCreature(state, 'player1', {
      templateId: 'cP',
      name: 'P',
      currentHealth: 3,
      health: 1,
      passiveHealthModifier: 2,
    });
  ...
  test('trigger chain third-party death is swept', () => {
    ...
    const attacker = pushCreature(state, 'player1', {
      templateId: 'atk',
      name: 'Attacker',
      attack: 2,
    });
    pushCreature(state, 'player2', {
      templateId: 'def',
      name: 'Defender',
      health: 1,
      currentHealth: 1,
    });
    const third = pushCreature(state, 'player2', {
      templateId: 'thr',
      name: 'Third',
      health: 1,
      currentHealth: 1,
