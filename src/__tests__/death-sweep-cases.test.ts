...
function pushCreature(state: GameState, owner: 'player1' | 'player2', card: Partial<FieldCard> & { id: string; name: string }): FieldCard {
...
describe('Death Sweep Edge Cases', () => {
  ...
  pushCreature(state, 'player1', { id: 'cA', name: 'A', currentHealth: 0 });
  pushCreature(state, 'player1', { id: 'cB', name: 'B', currentHealth: 0 });
    ...
    expect(state.players.player1.graveyard.map(c => c.id).sort()).toEqual(['cA','cB']);
  });

  test('passive recalculation induced death is swept', () => {
    const state = setupState();
    // 高い passiveHealthModifier を仮で与え、後で 0 に戻す過程を模擬
    const card = pushCreature(state, 'player1', { id: 'cP', name: 'P', currentHealth: 3, health: 1, passiveHealthModifier: 2 });
    ...
    expect(state.players.player1.field.find(c => c.id === 'cP')).toBeUndefined();
    expect(state.players.player1.graveyard.find(c => c.id === 'cP')).toBeDefined();
  });

  test('trigger chain third-party death is swept', () => {
    const state = setupState();
    // A (攻撃者), B (防御), X (第三者) を配置
  const attacker = pushCreature(state, 'player1', { id: 'atk', name: 'Attacker', attack: 2 });
  pushCreature(state, 'player2', { id: 'def', name: 'Defender', health: 1, currentHealth: 1 });
    const third = pushCreature(state, 'player2', { id: 'thr', name: 'Third', health: 1, currentHealth: 1 });
    ...
    evaluatePendingDeaths(state, 'trigger', attacker.id);
    expect(state.players.player2.field.find(c => c.id === 'thr')).toBeUndefined();
    expect(state.players.player2.graveyard.find(c => c.id === 'thr')).toBeDefined();
  });
});
