...
describe('stepGame', () => {
  ...
  it('actionLog に新規アクションが追加された場合は差分のみ返す', () => {
    ...
    const base: GameAction = { sequence: 1, playerId: P1, type: 'card_attack', data: { attackerCardId: 'cA', targetId: 'cB', damage: 2 }, timestamp: 0 } as unknown as GameAction;
    const next: GameState = { actionLog: [base, { sequence: 2, playerId: P1, type: 'effect_trigger', data: { sourceCardId: 'cX', effectType: 'damage', effectValue: 3, targets: { cB: { health: { before: 5, after: 2 } } } }, timestamp: 1 }], players: dummyPlayers() } as unknown as GameState;
