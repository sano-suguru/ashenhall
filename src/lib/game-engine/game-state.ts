...
export function createInitialGameState(
  ...
  const createPlayerState = (id: PlayerId, deck: Card[], faction: Faction): PlayerState => {
  ...
  return {
    ...
    phase: 'draw',
    players: {
      player1: createPlayerState('player1', player1Deck, player1Faction),
      player2: createPlayerState('player2', player2Deck, player2Faction),
    },
    actionLog: [
      {
        sequence: 0,
...
export function checkGameEnd(state: GameState): GameResult | null {
  ...
  if (state.turnNumber > 30) {
    const winner =
      player1.life > player2.life ? 'player1' : player2.life > player1.life ? 'player2' : null;
    return {
      winner,
      reason: 'timeout',
      totalTurns: state.turnNumber,
