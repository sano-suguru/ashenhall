...
export function useBattleLog(gameState: GameState) {
  ...
  const formatBattleLogAsText = useCallback(
    (useFiltered: boolean = false): string => {
      ...
      const grouped = actionsToFormat.reduce(
        (acc, action) => {
  ...
  const copyToClipboard = useCallback(
    async (useFiltered: boolean = false) => {
      try {
