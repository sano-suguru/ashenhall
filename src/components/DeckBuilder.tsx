        ...
        <header className="p-4 border-b border-gray-700 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 bg-green-600 hover:bg-green-500 rounded-lg transition-colors disabled:bg-gray-500 disabled:cursor-not-allowed"
            >
              <Save />
              <span>保存</span>
            </button>
        ...
        <div className="flex-grow flex overflow-hidden">
          ...
          <aside className="w-1/3 p-4 overflow-y-auto border-r border-gray-700">
            <h3 className="text-xl font-bold mb-4 text-amber-300">
              カード一覧 ({availableCards.length}種)
            </h3>
            <p className="text-sm text-gray-400 mb-4">
              ★アイコンクリックでコアカード(3枚まで)を選択
            </p>
                ...
                return (
                  <div key={card.templateId} className="relative">
                    ...
                    <div className="absolute top-2 right-2 flex items-center space-x-1">
                      <button
                        onClick={() => toggleCoreCard(card.templateId)}
                        className={`p-1 rounded-full ${isCore ? 'bg-amber-400 text-gray-900' : 'bg-black/70 text-gray-400 hover:text-white'}`}
                      >
                        <Star size={16} />
                      </button>
          ...
          <main className="w-2/3 p-4 flex flex-col" ref={deckContentRef}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-amber-300">
                デッキ内容 ({currentDeck.cards.length}/{GAME_CONSTANTS.DECK_SIZE})
              </h3>
                  ...
                  .map((cardId, index) => {
                    ...
                    return card ? (
                      ...
                      >
                        <CardComponent card={card} size="medium" />
                        {isCore && (
                          <Star size={16} className="absolute top-2 left-2 text-amber-400" />
                        )}
                      </div>
                    ) : null;
                  })}
